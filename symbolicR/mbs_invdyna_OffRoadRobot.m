...
% Backward Dynamics 
  ...
  Fs214 = -(s.frc(2,14)-s.m(14)*(ALPHA26+BETA46*s.dpt(1,4)+BETA66*s.dpt(3,4)+BS56*s.dpt(2,4)+s.l(3,14)*(OM214*OM314+C14*(qd(14)*OM313-C13*(OMp16-...
 qd(13)*OM36)+S13*(OMp36+qd(13)*OM16))+S14*(qd(14)*OM113+C13*(OMp36+qd(13)*OM16)+S13*(OMp16-qd(13)*OM36)))));
  ...
  Fs212 = -(s.frc(2,12)-s.m(12)*(ALPHA26+BETA46*s.dpt(1,3)+BETA66*s.dpt(3,3)+BS56*s.dpt(2,3)+s.l(3,12)*(OM212*OM312+C12*(qd(12)*OM311-C11*(OMp16-...
 qd(11)*OM36)+S11*(OMp36+qd(11)*OM16))+S12*(qd(12)*OM111+C11*(OMp36+qd(11)*OM16)+S11*(OMp16-qd(11)*OM36)))));
  ...
  Fs210 = -(s.frc(2,10)-s.m(10)*(ALPHA26+BETA46*s.dpt(1,2)+BETA66*s.dpt(3,2)+BS56*s.dpt(2,2)+s.l(3,10)*(OM210*OM310+C10*(qd(10)*OM39-C9*(OMp16-...
 qd(9)*OM36)+S9*(OMp36+qd(9)*OM16))+S10*(qd(10)*OM19+C9*(OMp36+qd(9)*OM16)+S9*(OMp16-qd(9)*OM36)))));
  ...
  Fs28 = -(s.frc(2,8)-s.m(8)*(ALPHA26+BETA46*s.dpt(1,1)+BETA66*s.dpt(3,1)+BS56*s.dpt(2,1)+s.l(3,8)*(OM28*OM38+C8*(qd(8)*OM37-C7*(OMp16-qd(7)*OM36...
 )+S7*(OMp36+qd(7)*OM16))+S8*(qd(8)*OM17+C7*(OMp36+qd(7)*OM16)+S7*(OMp16-qd(7)*OM36)))));
  ...
  Cq16 = -(s.trq(1,6)-s.In(1,6)*OMp16-Cq111*C11-Cq113*C13-Cq17*C7-Cq19*C9-Cq311*S11-Cq313*S13-Cq37*S7-Cq39*S9+Fs210*s.dpt(3,2)+Fs212*s.dpt(3,3)+...
 Fs214*s.dpt(3,4)+Fs28*s.dpt(3,1)+OM26*OM36*(s.In(5,6)-s.In(9,6))+s.dpt(2,1)*(Fq17*S7-Fq37*C7)+s.dpt(2,2)*(Fq19*S9-Fq39*C9)+s.dpt(2,3)*(Fq111*S11-...
 Fq311*C11)+s.dpt(2,4)*(Fq113*S13-Fq313*C13));
  Cq26 = -(s.trq(2,6)-Cq210-Cq212-Cq214-Cq28-s.In(5,6)*OMp26-OM16*OM36*(s.In(1,6)-s.In(9,6))-s.dpt(1,1)*(Fq17*S7-Fq37*C7)-s.dpt(1,2)*(Fq19*S9-...
 Fq39*C9)-s.dpt(1,3)*(Fq111*S11-Fq311*C11)-s.dpt(1,4)*(Fq113*S13-Fq313*C13)-s.dpt(3,1)*(Fq17*C7+Fq37*S7)-s.dpt(3,2)*(Fq19*C9+Fq39*S9)-s.dpt(3,3)*(...
 Fq111*C11+Fq311*S11)-s.dpt(3,4)*(Fq113*C13+Fq313*S13));
  Cq36 = -(s.trq(3,6)-s.In(9,6)*OMp36+Cq111*S11+Cq113*S13+Cq17*S7+Cq19*S9-Cq311*C11-Cq313*C13-Cq37*C7-Cq39*C9-Fs210*s.dpt(1,2)-Fs212*s.dpt(1,3)-...
 Fs214*s.dpt(1,4)-Fs28*s.dpt(1,1)+OM16*OM26*(s.In(1,6)-s.In(5,6))+s.dpt(2,1)*(Fq17*C7+Fq37*S7)+s.dpt(2,2)*(Fq19*C9+Fq39*S9)+s.dpt(2,3)*(Fq111*C11+...
 Fq311*S11)+s.dpt(2,4)*(Fq113*C13+Fq313*S13));
